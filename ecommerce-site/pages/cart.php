...
session_start();
define('SITE_ACCESS', true); // ← Ajoute cette ligne ici


//Inclusion de l'en-tête (connexion DB + session)
...
if (isset($_POST['update_quantity'])) {
    ...
    if ($new_quantity > 0) {
        $sql = "UPDATE cart SET quantite = ? WHERE user_id = ? AND product_id = ?";
        $stmt = mysqli_prepare($conn, $sql);
...
$sql = "SELECT c.id as cart_id, c.quantite, c.product_id,
               p.nom, p.prix, p.image, p.stock,
               (c.quantite * p.prix) as subtotal
        FROM cart c
